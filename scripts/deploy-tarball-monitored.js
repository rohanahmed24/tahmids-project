...
conn.on('ready', () => {
    ...
    function backupStandalone(callback) {
        ...
        const cmd = `cd ${remoteProject} && cp -r .next/standalone .next/standalone.backup.${backupTimestamp} 2>/dev/null || echo "No standalone to backup"`;
    ...
    function rollbackStandalone(callback) {
        ...
        console.log(`🔄 Restoring backup .next/standalone.backup.${backupTimestamp}`);
        const cmd = `cd ${remoteProject} && rm -rf .next/standalone && cp -r .next/standalone.backup.${backupTimestamp} .next/standalone`;
        conn.exec(cmd, (err, stream) => {
            if (err) {
    ...
    function logCpuUsage(callback) {
        ...
        conn.exec(cmd, (err, stream) => {
            if (err) {
                console.log('⚠️ Could not get CPU usage:', err.message);
                callback();
                return;
            }
            stream.on('data', (data) => {
