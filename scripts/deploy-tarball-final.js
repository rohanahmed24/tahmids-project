...
conn.on('ready', () => {
    ...
    // Helper for exec with timeout and data capture
    function execWithTimeout(cmd, timeoutMs, callback) {
        const timeout = setTimeout(() => {
            console.error(`⏰ Timeout after ${timeoutMs}ms for command: ${cmd}`);
    ...
    function stopPm2(callback) {
        ...
        const cmd = 'cd ' + remoteProject + ' && pm2 stop wisdomia 2>/dev/null || true';
        execWithTimeout(cmd, 30000, (err, code) => {
            if (err) {
                callback(err);
                return;
            }
    ...
    function extractTarball(callback) {
        ...
        execWithTimeout(cmd, 60000, (err, code) => {
            if (err) {
                callback(err);
                return;
            }
    ...
    function installDeps(callback) {
        ...
        execWithTimeout(cmd, 120000, (err, code) => {
            if (err) {
                callback(err);
                return;
            }
