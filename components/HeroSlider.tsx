...
export function HeroSlider({ items }: HeroSliderProps) {
  ...
  const copy =
    locale === "bn"
      ? {
          ...
          discover:
            "ভাবনাকে নাড়া দেয় এমন লেখা, অন্তর্দৃষ্টি ও গল্প আবিষ্কার করুন।",
          hotTopics: "গরম বিষয়",
      ...
      <section className="relative w-full h-[50vh] md:h-screen overflow-hidden bg-gradient-to-br from-purple-900 via-indigo-900 to-black">
        ...
        <div className="absolute inset-0 flex flex-col items-center justify-center text-center px-6">
          ...
          <h1 className="text-4xl md:text-7xl font-serif font-bold text-white mb-4 max-w-4xl">
            {copy.source}{" "}
            <span className="italic text-purple-300">{copy.source2}</span>
          </h1>
  ...
  return (
    ...
    >
        ...
        <div className="flex h-full touch-pan-y">
          {validItems.map((item, index) => (
            ...
            >
              ...
              <div
                className={`absolute inset-0 bg-gradient-to-r ${item.accent_color || "from-blue-600/30 to-purple-600/30"} mix-blend-soft-light opacity-50`}
              />
            </div>
          ))}
        </div>
      ...
      <div className="hidden md:block absolute top-32 left-12 z-20">
      ...
      <div className="hidden lg:flex absolute bottom-12 left-0 right-0 justify-center z-20 gap-3 pointer-events-auto">
        {validItems.map((topic, index) => (
          <button
            key={topic.slug}
            onClick={() => scrollTo(index)}
            className={`relative w-20 h-14 rounded-lg overflow-hidden transition-all hover:-translate-y-1 hover:scale-105 ${
              index === selectedIndex
