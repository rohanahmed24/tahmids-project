      ...
      <div className="max-w-[1600px] mx-auto space-y-16 text-center">
        <MotionWrapper type="slide-up">
          <div className="space-y-4 flex flex-col gap-1">
            <span className="text-text-primary text-xs md:text-sm font-sans font-bold tracking-[0.2em] uppercase opacity-60">
              {t(locale, "discovery")}
            </span>
            <h2 className="text-4xl md:text-5xl lg:text-6xl font-serif font-medium text-text-primary">
              {locale === "bn" ? "বিষয়ভিত্তিক" : "Explore by"}{" "}
        ...
        <div className="md:hidden grid grid-cols-2 gap-3">
          {categories.map((topic) => (
            ...
            >
              <span className="text-base font-sans font-bold text-text-primary">
                {topic.name}
              </span>
              <span className="text-[10px] font-bold tracking-widest text-text-muted uppercase">
                {formatCount(topic.count, locale)}
              </span>
            </Link>
          ))}
        </div>
        ...
        <MotionWrapper
          ...
          className="hidden md:flex md:flex-wrap md:justify-center lg:justify-between gap-6 2xl:gap-10"
        >
          {categories.map((topic) => (
            <MotionItem key={topic.slug} className="group">
              ...
              >
                ...
                <div className="relative z-10 text-center space-y-2">
                  <span className="text-2xl 2xl:text-3xl font-sans font-bold text-text-primary group-hover:text-[#000000] transition-colors block">
                    {topic.name}
                  </span>
                  <span className="text-xs font-bold tracking-widest text-text-primary/70 uppercase block group-hover:text-[#000000] transition-colors">
                    {formatCount(topic.count, locale)}
                  </span>
                </div>
              </Link>
        ...
        <MotionWrapper type="fade-in" delay={0.4} className="pt-8">
          <Link
            href="/topics"
            className="font-bold font-sans text-sm md:text-base border-b-2 border-text-primary pb-1 hover:text-text-primary/70 hover:border-text-primary/70 transition-all"
          >
